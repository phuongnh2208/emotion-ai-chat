 ...
 * Structured safety guidelines for AI responses
 */
...
export function buildSafetyPrompt(emotion = "neutral", isFirstMessage = false) {
  const emotionVi = getEmotionVietnamese(emotion);
...
${getFirstMessageGuidance(isFirstMessage, emotionVi)}
...
/**
 * Get Vietnamese translation for emotion
...
function getEmotionVietnamese(emotion) {
  ...
  return emotionMap[emotion] || "bình thường";
}

/**
 * Get guidance for first message
 * @param {boolean} isFirstMessage - Is first message
 * @param {string} emotionVi - Emotion in Vietnamese
 * @returns {string} Guidance text
 */
function getFirstMessageGuidance(isFirstMessage, emotionVi) {
  if (!isFirstMessage) {
    return `
HỌC SINH ĐÃ TRẢ LỜI:
- Trả lời dựa trên NỘI DUNG học sinh vừa viết
